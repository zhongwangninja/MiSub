...
function generateClashConfig(nodesString) {
    // 1. 解析和过滤节点，确保只处理有效且包含必要信息的节点
    const proxies = nodesString.split('\n')
        .map(link => parseNodeLink(link.trim()))
        .filter(p => p && p.name && p.server && p.port); // 过滤掉无效或不完整的节点

    if (proxies.length === 0) {
        return "proxies: []"; // 如果没有有效节点，返回一个空的proxies列表
    }

    // 2. 将节点名称列表用于生成 proxy-groups
    const proxyNames = proxies.map(p => p.name.replace(/"/g, '')); // 移除名称中的引号以防YAML格式错误

    // 3. 生成每个节点的详细配置
    const proxyDetails = proxies.map(p => {
        // 为不同协议设置最基础的必要参数
        let base_config = {
            name: `"${p.name.replace(/"/g, '')}"`, // 确保名称是带引号的字符串
            type: p.protocol,
            server: p.server,
            port: p.port,
        };
        ...
        switch (p.protocol) {
            case 'vmess':
            case 'vless':
                // 对于 Vmess/Vless，补充必要的默认值以避免错误
                base_config = {
                    ...base_config,
                    uuid: "00000000-0000-0000-0000-000000000000", // 使用占位符UUID
                    alterId: 0,
                    cipher: "auto",
                    tls: true,
                    network: "ws",
            ...
            case 'ss':
                 base_config = {
                    ...base_config,
                    cipher: p.cipher || "aes-256-gcm", // 从解析器或默认值获取
                    password: p.password || "password",
                };
                break;
        }

        // 将对象转换为 "key: value" 的格式，并用逗号+空格连接
        const formatted_config = Object.entries(base_config).map(([key, value]) => `${key}: ${value}`).join(', ');
