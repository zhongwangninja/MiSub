...
<template>
  <div class="w-full max-w-md">
    <div class="bg-white dark:bg-gray-900/80 backdrop-blur-sm shadow-2xl rounded-2xl p-10">
      ...
      <form @submit.prevent="submitLogin" class="mt-8 space-y-6">
        <div class="relative">
          ...
          <input 
            ...
            required 
            class="w-full pl-10 pr-3 py-3 bg-gray-100 dark:bg-gray-800 border-2 border-transparent rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500 dark:text-white" 
            placeholder="请输入密码"
          >
        </div>
        ...
        <div>
          <button 
            type="submit" 
            :disabled="isLoading"
            class="w-full flex justify-center py-3 px-4 border border-transparent rounded-lg shadow-sm text-sm font-medium text-white bg-indigo-600 hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 disabled:opacity-75 transition-opacity"
          >
