...
<template>
  ...
  >
    ...
    <div class="mt-3 flex-grow flex flex-col justify-center space-y-2">
        <input
            type="text"
            :value="misub.url"
            readonly
        ...
        <div v-if="trafficInfo" class="space-y-1 pt-1">
            ...
            <div class="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-2">
                <div class="bg-gradient-to-r from-blue-500 to-indigo-600 h-2 rounded-full" :style="{ width: trafficInfo.percentage + '%' }"></div>
            </div>
        </div>
    </div>
    ...
    <div class="flex justify-between items-center mt-4">
        <div class="flex items-center gap-2">
          ...
          <span v-if="expiryInfo" class="text-xs font-medium" :class="expiryInfo.style">
      ...
      <div class="flex items-center space-x-3">
        ...
        <button @click="emit('update')" :disabled="misub.isUpdating" class="text-gray-400 hover:text-indigo-500 transition-colors disabled:opacity-50 disabled:cursor-not-allowed" title="更新节点数和流量">
